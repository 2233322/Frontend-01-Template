...
function match(element, selector) {
  ...
  if (selector.charAt(0) === '#') {
    ...
    if (attr && attr.value.split(' ').length > 1) {
      ...
      //  .a.b    class='a b c'    attrSplit.length === selectorSplit.length 可以不用相等
      //  应该是 selectorSplit 中的每个元素都能在 attrSplit中能找到 你的写反了
      // 
      // if (attrSplit.length === selectorSplit.length) {
      //   return attrSplit.every((item) => {
      //     return selectorSplit.indexOf(item) > -1
      //   })
      // } else {
      //   return false
      // }

      return selectorSplit.every((item) => {
        return attrSplit.indexOf(item) > -1
      })
